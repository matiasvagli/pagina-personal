...
export const notasTecnicas: NotaTecnica[] = [
  {
    titulo: "Por qué usar el patrón Repository en backend",
    fecha: "Nov 2025",
    resumen: "El patrón Repository aísla la lógica de acceso a datos de la lógica de negocio. Simplifica testing, facilita cambios de base de datos y mejora la mantenibilidad. Una guía corta y práctica.",
    contenido: `
      <h2>¿Qué es el patrón Repository?</h2>
      <p>El patrón Repository es un intermediario entre tu lógica de negocio y la base de datos. En lugar de que tu controlador o servicio acceda directo a la BD, se comunica a través de un Repository que encapsula todas las operaciones CRUD.</p>

      <h3>❌ Sin Repository (Acoplamiento)</h3>
      <pre><code>class VentasService:
    def crear_venta(self, cliente_id, monto):
    ...
    def buscar_ventas(self, cliente_id):
        # ❌ Otra vez accediendo a BD directamente
        return Venta.objects.filter(cliente_id=cliente_id)</code></pre>

      <h4>Problemas:</h4>
      <ul>
        <li>❌ Si cambias de BD, tienes que tocar todo el código</li>
        <li>❌ Difícil de testear (necesitas BD real)</li>
        <li>❌ Lógica de BD y negocio mezcladas</li>
        <li>❌ No reutilizable</li>
      </ul>

      <h3>✅ Con Repository (Desacoplamiento)</h3>
      <pre><code>class VentasRepository:
    # 🔒 Todo el acceso a BD en un lugar
    def crear(self, cliente_id, monto):
...
class VentasService:
    ...
    def registrar_venta(self, cliente_id, monto):
        # ✅ Lógica de negocio pura
        if monto < 0:
            raise ValueError("Monto negativo")
        return self.repo.crear(cliente_id, monto)

    def obtener_historial(self, cliente_id):
        return self.repo.buscar_por_cliente(cliente_id)</code></pre>

      <h3>✅ Beneficios del patrón Repository</h3>
      <table>
        <thead>
          <tr>
            <th>Beneficio</th>
        ...
        <tbody>
          <tr>
            <td>Fácil de testear</td>
            <td>Podés pasar un Repository fake sin BD real</td>
          </tr>
          <tr>
            <td>Cambiar BD es simple</td>
            <td>Solo reemplazas el Repository</td>
          </tr>
          <tr>
            <td>Código más limpio</td>
            <td>Lógica de BD separada de lógica de negocio</td>
          </tr>
          <tr>
            <td>Reutilizable</td>
            <td>Mismo Repository para múltiples servicios</td>
          </tr>
          <tr>
            <td>Abstracción profesional</td>
            <td>Interfaces claras entre capas</td>
          </tr>
        </tbody>
      </table>

      <h3>🧪 Testing sin BD</h3>
      <pre><code># Test con Repository Fake (sin BD real)
class VentasRepositoryFake:
    def __init__(self):
        self.ventas = []

    def crear(self, cliente_id, monto):
        venta = {"id": 1, "cliente_id": cliente_id, "monto": monto}
        self.ventas.append(venta)
        return venta

# Test
repo_fake = VentasRepositoryFake()
servicio = VentasService(repo_fake)

def test_registrar_venta():
    resultado = servicio.registrar_venta(cliente_id=5, monto=1000)
    assert resultado["monto"] == 1000  # ✅ Test rápido, sin BD</code></pre>

      <h3>🔄 Cambiar de BD es fácil</h3>
      <pre><code># Opción A: PostgreSQL
class VentasRepositoryPostgres:
    def crear(self, cliente_id, monto):
        # Código para PostgreSQL
        pass

# Opción B: MongoDB
class VentasRepositoryMongo:
    def crear(self, cliente_id, monto):
        # Código para MongoDB
        pass

# El servicio NO cambia de código
servicio_pg = VentasService(VentasRepositoryPostgres())
servicio_mongo = VentasService(VentasRepositoryMongo())</code></pre>

      <h3>📐 Estructura recomendada</h3>
      <pre><code>proyecto/
├── servicios/
│   └── ventas_service.py      # Lógica de negocio
├── repositories/
│   ├── __init__.py
│   ├── base_repository.py     # Clase base
│   └── ventas_repository.py   # Implementación específica
├── modelos/
│   └── venta.py               # Modelo ORM
└── controladores/
    └── ventas_controller.py   # Endpoints</code></pre>

      <h3>🏗️ Base Repository (reutilizable)</h3>
...
class VentasRepository(BaseRepository):
    def crear(self, cliente_id, monto):
        return Venta.objects.create(cliente_id=cliente_id, monto=monto)
    ...
    url: "/notas/por-que-usar-patron-repository-backend"
  },
  {
